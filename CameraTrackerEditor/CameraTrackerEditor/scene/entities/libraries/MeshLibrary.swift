...
class MeshLibrary: GraphicsLibrary<MeshTypes, Mesh> {
...
class ModelMesh: Mesh {
    ...
    func loadModel(modelName: String) {
        ...
        let bufferAllocator = MTKMeshBufferAllocator(device: SceneEngine.device)
        let asset: MDLAsset = MDLAsset(url: assetURL,
                                       vertexDescriptor: descriptor,
                                       bufferAllocator: bufferAllocator)
        do{
            self._meshes = try MTKMesh.newMeshes(asset: asset,
                                                 device: SceneEngine.device).metalKitMeshes
        } catch {
...
class CustomMesh: Mesh {
    ...
    func createBuffers(){
        _vertexBuffer = SceneEngine.device.makeBuffer(bytes: _vertices,
